...
export default function GestorObras() {
  ...
  return (
    <>
      ...
      {/* MODALES */}
      {modal === 'obra'      && <ModalObra      itemEdit={itemEditando} clientes={clientes} onClose={cerrarModal} onGuardar={async d => { if (!d.nombre) return alert('El nombre es obligatorio'); const { id, nombre, cliente_id, estado, presupuesto } = d; const payload = { nombre, cliente_id: cliente_id || null, estado, presupuesto: parseFloat(presupuesto) || 0 }; const res = id ? await supabase.from('obras').update(payload).eq('id', id) : await supabase.from('obras').insert([payload]); if (res.error) alert('Error: ' + res.error.message); else { cerrarModal(); recargarObras() } }} />}
